...
class Ref:
    ...
    _thread: threading.Thread = attr.ib(init=False, validator=instance_of(threading.Thread), default=threading.current_thread())

    async def tell(self, message: Any):
        if threading.current_thread() is not self._thread:
            await asyncio.get_event_loop().run_in_executor(None, lambda: self.inbox.sync_q.put(message))
        else:
