...
class Probe:
    ...
    async def expect_message(self, msg, timeout: timedelta = timedelta(seconds=10)) -> bool:
        async def _check():
            while True:
...
class ActorTestKit:
    ...
    async def spawn(self, factory, id: Optional[str] = None) -> Ref:
        return await self.system.ask(lambda reply_to: Spawn(reply_to=reply_to, id=id, factory=factory))
    
    async def create_probe(self) -> Ref:
        ref = Ref(id=str(uuid.uuid1()), inbox=janus.Queue(), thread=threading.current_thread())
